...
void grosor(double * s, double * f, int n, int dim, int xsize, int ysize, int cota_superior, int cota_inferior, double *image) {
	...
	/* Definiciones para conocer si son medidas buenas 
	 * o malas, varianza y desviacion estandar
	 */
	int mi = 0; /* indice del vector medidas */
	int mi_old = 0; /* indice del vector medidas */
	int cant_old=1, sum_old=0;
	int k_old=0; /* para conocer si se hicieron las 3 mediciones en un segmento */
	...
	for(i=0; i<n; i++) {
		...
		/* Para no medir en la medula y en segmentos muy chicos */
		d = sqrt(  pow((Mx - mx),2) + pow((My-my ),2) );
		if (d < seg_muy_chico) continue;
		pb = s[i*dim+1] - (   ((-1) * 1/f[i*2] ) * s[i*dim+0]  );
		pp = (-1) * 1/f[i*2] ;

		/* para conocer si son medidas buenas o malas */
		sum_old = sum;
		cant_old = cant;
		mi_old = mi;
		k_old = 0;
